...
int checkAdventurer(int player, struct gameState *post, int deckSize, int discardSize, int c) {
    ...
    // printf("Trying          Player: %d, Case: %d, Expected Coins: %d, Actual Coins: %d, Deck Size: %d, Discard Size: %d, Hand Count: %d\n",player, c, (c + 1), post->coins, deckSize, discardSize, post->handCount[player]);
            ...
            for(handCount = 1; handCount < 10; handCount++) {
                ...
                for(discardCount = 4; discardCount < 10; discardCount++) {
                        for(c = 0; c <= 3; c++) {
                            ...
                            if(c == 0)
                                G.discard[p][1] = 4;
                            // Case 1: Treasures all from discard
                            else if(c == 1) {
                                G.discard[p][1] = 4;
                                G.discard[p][2] = 4;
                            // Case 2: Treasures from each deck and discard
                            } else if(c == 2) {
                                G.deck[p][1] = 4;
                                G.discard[p][2] = 5;
                            ...
                            // Case 0: 1 Treasure total, from discard
                            if(c == 0)
                                memset(G.discard[p], 4, sizeof(int));
                            // Case 1: Treasures all come from discard
                            else if(c == 1) {
                                memset(G.deck[p], 4, sizeof(int) * 2);
                                // Case 2: 1 Treasure from each deck and discard
                            } else if(c == 2) {
                                memset(G.deck[p], 4, sizeof(int));
                                memset(G.discard[p], 5, sizeof(int));
                                // Case 3: Treasures all come from deck
                            } else if(c == 3) {
                                memset(G.deck[p], 5, sizeof(int) * 2);
                                memset(G.discard[p], 6, sizeof(int) * discardCount);
                            }
                             */
