...
int checkSmithy(int player, struct gameState *post, int handPos, int handSize, int deckSize) {
    ...
    playSmithy(post, player, handPos);
    
    pre.deckCount[player] = pre.deckCount[player] - 3;
    pre.handCount[player] = pre.handCount[player] + 2;
    
    if(pre.handCount[player] != post->handCount[player] || pre.deckCount[player] != post->deckCount[player]) {
        printf("TEST FAILED          Player: %d, New Hand Size (Theor): %d, New Hand Size (Actual): %d, Deck Size (Theor): %d, Deck Size (Actual): %d, Hand Position: %d\n",player,
               pre.handCount[player], post->handCount[player], pre.deckCount[player], post->deckCount[player], handPos);
        return -1;
    } else {
        // printf("TEST OK\n");
        return 0;
...
int main(int argc, const char * argv[]) {
    ...
    for(n = 0; n < 20000; n++) {
        ...
        deckSize = G.deckCount[p] = floor(Random() * (MAX_DECK - 5)) + 5;
        discardSize = G.discardCount[p] = floor(Random() * (MAX_DECK - 5)) + 5;
        G.playedCardCount = 0;
