...
{
    // track game state and current player

    int drawntreasure=0; // track how much treasure has been drawn
    int temphand[MAX_HAND]; // temp int array to track cards
    int cardDrawn;
    int tempcounter=0;
    ...
    while(drawntreasure<2){
	  ...
	  break;
        if (state->deckCount[currentPlayer] <1 && state->discardCount[currentPlayer] >=1){//if the deck is empty we need to shuffle discard and add to deck
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case baron:
        return playBaron(state, currentPlayer, handPos);
