...
int checkFeast(int player, struct gameState *post, int choice, int currSupply, int handPos) {
    struct gameState pre;
    memcpy (&pre, post, sizeof(struct gameState));
    
    // Added call to cardEffect function
    cardEffect(feast, choice, 0, 0, post, handPos, 0);
...
int main() {
    ...
    int p, x, handCount, handPos, currSupply, testVal;
        ...
        for(x = 0; x < 10; x++) {
            ...
            for(handCount = 1; handCount < 10; handCount++) {
                ...
                for(currSupply = 10; currSupply >= 0; currSupply--) {
                    testVal = checkFeast(p,&G, k[x], currSupply, handPos);
                    if(testVal == -1)
                        testsOk = 0;
                }
